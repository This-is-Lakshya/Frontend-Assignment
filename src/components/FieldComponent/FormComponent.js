...
const FormComponent = ({ schema, toggleField })=>{
  ...
  const renderSubParameters = (fields)=>{
    return fields.map((schema)=>{
      if (schema.uiType === "Ignore") {
        if (schema.conditions.some((condition)=>
          ...
          if(schema.validate.required !== true){
            return  toggleField ? <SelectField schema={schema} /> : null ;
          }
